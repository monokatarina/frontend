...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Ao iniciar, verificar se há um usuário salvo no localStorage e tentar logar automaticamente
    const savedUser = localStorage.getItem('user');
    if (savedUser) {
        try {
            ...
            setTimeout(() => {
                ...
                // Também verificar após 1, 3 e 5 segundos
                setTimeout(() => forceUserUpdate(), 1000);
                setTimeout(() => forceUserUpdate(), 3000);
                setTimeout(() => forceUserUpdate(), 5000);
            }, 500);
...
// ============================================
// FUNÇÃO PARA VERIFICAR E PROCESSAR PENDING PLANS
// ============================================
async function checkAndProcessPendingPlans() {
    if (!currentUser) return;
    
    try {
        console.log('🔍 Verificando pendingPlans...');
        ...
        if (response.ok) {
            ...
            console.log('📦 Dados completos do usuário:', {
                ...
                pendingPlans: userData.pendingPlans,
                pendingPayment: userData.pendingPayment
            });
            ...
            if (userData.pendingPlans && userData.pendingPlans.length > 0) {
                ...
                if (userData.pendingPayment) {
                    ...
                    if (statusResponse.ok) {
                        ...
                        if (statusData.status === 'approved') {
                            console.log('✅ Pagamento aprovado! Forçando atualização...');
                            ...
                            setTimeout(async () => {
                                ...
                                // Verificar novamente se os planos foram ativados
                                const checkResponse = await fetch(`${API}/auth/me?userId=${currentUser.id}`, {
                                    credentials: 'include',
                                    headers: { 
                                        'Content-Type': 'application/json',
                                        'X-User-ID': currentUser.id
                                ...
                                if (checkResponse.ok) {
                                    ...
                                    const updatedUser = checkData.user || checkData.data || checkData;
                                    
                                    if (updatedUser.plans && updatedUser.plans.length > 0) {
                                        console.log('✅ Planos ativados!', updatedUser.plans);
                                        
                                        // Atualizar currentUser
                                        currentUser.plans = updatedUser.plans;
                                        delete currentUser.pendingPlans;
                                        localStorage.setItem('user', JSON.stringify(currentUser));
            ...
            if (!userData.pendingPlans && userData.plans && userData.plans.length > 0) {
                if (!currentUser.plans || currentUser.plans.length === 0) {
                    console.log('✅ Planos ativos encontrados! Atualizando...');
                    currentUser.plans = userData.plans;
                    localStorage.setItem('user', JSON.stringify(currentUser));
                    updatePlanInfo();
                    await loadData();
        ...
        console.error('❌ Erro ao verificar pendingPlans:', error);
    }
}
...
async function forceUserUpdate() {
    ...
    try {
        ...
        const response = await fetch(`${API}/auth/me?userId=${currentUser.id}&_=${Date.now()}`, {
            credentials: 'include',
            headers: { 
                'Content-Type': 'application/json',
                'X-User-ID': currentUser.id,
...
function startAutoCheck() {
    ...
    const interval = setInterval(async () => {
        ...
        await checkAndProcessPendingPlans();
    }, 5000);
